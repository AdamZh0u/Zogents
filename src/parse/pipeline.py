...
# I wanted to download them prior to running the pipeline and created this crutch
    ...
    def parse_pdf_reports_sequential(self, input_doc_paths=None):
        logging.basicConfig(level=logging.DEBUG)
        ...
        if input_doc_paths is not None:
            pdf_parser.parse_and_export(input_doc_paths=[Path(p) for p in input_doc_paths])
        else:
